...
# This code is improvement over the actual code "pxp7948_LAB3.py".   #
# The additional operator that this code supports is the modulo      #
# operator (%). As the expression can result in float values, I have #
# used math.fmod to implement the same in this code.                 #
#--------------------------------------------------------------------#
...
# Function to convert the infix expression to postfix notation
def convert_infix_2_postfix(infix):
    # Initialize the result string to be blank
    result_postfix = ""
    # Initialize stack
    operations_stack = []
    ...
    for char in infix:
        # if the char is digit then directly add it to the output string
        if char.isdigit():
            result_postfix += char + " "
        # if the char is operator then
        elif char in operators:
            # If the character is an operator, all operators in the opstack list with 
            # higher or equal precedence are popped off and added to the postfix string 
            # until an operator with lower precedence is reached. The current operator is 
            # then added to the opstack list.
            while operations_stack and operations_stack[-1] in operators and operators[char] <= operators[operations_stack[-1]]:
                result_postfix += operations_stack.pop() + " "
            operations_stack.append(char)
        # if char is opening bracket then add it to the stack
        elif char == "(":
            operations_stack.append(char)
        # if char is closing bracket then pop all contents of stack till opening bracket is found
        elif char == ")":
            while operations_stack and operations_stack[-1] != "(":
                result_postfix += operations_stack.pop() + " "
            if operations_stack and operations_stack[-1] == "(":
                operations_stack.pop()
    # After all char are processed, any remaining operators are popped and added to the result string
    while operations_stack:
        result_postfix += operations_stack.pop() + " "
    # return the resulting postfix string
    return result_postfix.strip()
...
def main():
    ...
    with open(os.path.join(os.getcwd(), '.\input_RPN_EC.txt'), 'r') as file:
    ...
    for rpn_in in rpn_expressions:
        ...
        print("\nCalculations for Line " + str(counter))
        print("Infix expression:", rpn_in)

        # 0. Call the function to convert the infix expression in input file
        # to postfix notation
        rpn_in = convert_infix_2_postfix(rpn_in)
        print("Postfix expression:", rpn_in)
